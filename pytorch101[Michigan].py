...
def count_tensor_elements(x: Tensor) -> int:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    num_elements=int(torch.prod(Tensor(list(x.shape)),dim=0))
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x=torch.arange(start,start*(stop-start//10),10)
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y=x.clone()
    temp=-1*np.array(range(1,x.shape[0]+1))
    y=y[temp,:]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y=torch.zeros((len(x),np.max(x)+1))
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y=x.clone()
    y=y.reshape(2,3,4).permute(1,0,2).reshape(3,8)
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y=x.clone().reshape((-1,x.shape[-1]))
    index=torch.argmin(y,dim=-1)
    y[torch.arange(y.shape[0]),index]=0
    y=y.reshape(x.shape)
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    z=torch.zeros((x.shape[-3],x.shape[-2],y.shape[-1]),dtype=x.dtype)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y=x.clone()
    meany=torch.sum(y,dim=0)
    meanysq=torch.sum((y**2),dim=0)
    std=np.sqrt(meanysq-(meany**2)/(x.shape[0]-1))
    print(meany)
    y=(y-(meany/x.shape[0]))/(std)
