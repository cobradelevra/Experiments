...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test=x_test.reshape(num_test,-1)
    for i in range(num_train):
        dists[i]=torch.sqrt(torch.sum((x_train[i]-x_test)**2,dim=1))
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train=x_train.reshape(num_train,-1)
    x_test=x_test.reshape(num_test,-1)
    train2=torch.sum(torch.square(x_train),dim=1).reshape(num_train,1)
    test2=torch.sum(torch.square(x_test),dim=1)
    test2.repeat((num_train,1))
    dists=torch.sqrt(test2+train2-2*x_train@x_test.T)
...
(torch.chunk(dists,3)[1])

torch.topk(dists,3,largest=False,dim=0)[1][:,0]

torch.bincount(torch.tensor([1,1,2,2,2,0,4]))

torch.argmax(torch.bincount(torch.tensor([1,1,2,10,2,2,0,1])))
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    indicies=torch.topk(dists,k,largest=False,dim=0)[1]
    for i in range(num_test):
        y_pred[i]=torch.argmax(torch.bincount(y_train[indicies[:,i]]))
...
def knn_cross_validate(
    ...
    for i in range(num_folds):
        x_train_folds.append(len(torch.chunk(x_train,num_folds)[i]))
        y_train_folds.append(len(torch.chunk(y_train,num_folds)[i]))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for i in range(num_folds):
        knn=KnnClassifier(x_train[sum(x_train_folds[:i+1]):sum(x_train_folds[:i+2])],y_train[sum(y_train_folds[:i+1]):sum(y_train_folds[:i+2])])
        for j in k_choices:
            try : 
                k_to_accuracies[j].append([knn.check_accuracy(x_test,y_test,k=k)])
            except:
                k_to_accuracies[j]=[knn.check_accuracy(x_test,y_test,k=k)]
    #######
    ###################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
